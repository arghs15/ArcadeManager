...
class Themes:
    def __init__(self, parent_tab):
        ...
        self.image_folder = os.path.join(self.base_path, "collections", "settings", "images")
    ...
    def show_current_theme(self):
        ...
        _, image_path = self.themes_list[self.current_theme_index]

        # Check if the image exists before updating the canvas
        if os.path.isfile(image_path):
            self.update_image_canvas(image_path)
        else:
            print(f"Warning: No image found for theme at index {self.current_theme_index}, skipping display.")
            self.image_canvas.delete("all")  # Clear the canvas if no image is found
    ...
    def update_image_canvas(self, image_path):
        """Update the canvas with the provided image."""
        # Open the image
