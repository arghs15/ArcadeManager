...
class ConfigManager:
    ...
    CONFIG_FILE_VERSION = "3.1.1"  # Current configuration file version
    ...
    AVAILABLE_SETTINGS = {
        ...
        'ButtonVisibility': {
            ...
            'remove_roms_not_in_txt_button': {
                'default': 'never',  # 'always', 'never'
                'description': 'Controls visibility of Log Viewer button',
                'type': str,
                'hidden': True
            },
            'rom_operations_button': {
                'default': 'always',  # 'always', 'never'
                'description': 'Controls visibility of Log Viewer button',
                'type': str,
                'hidden': True
...
class ViewRoms:
    ...
    def create_buttons(self):
        button_configs = {
            ...
            'remove_roms_not_in_txt_button': {
                'text': "Remove Unwanted ROMs",
                'command': self.remove_roms_not_in_txt
            },
            # Combined button for ROM operations
            'rom_operations_button': {
                'text': "ROM Operations",
                'command': self.show_rom_operations_selection
            },
            # New button for checking ROMs in collections
            'check_roms_button': {
                'text': "Check ROMs",
                'command': self.check_roms_in_collection
            }
        }
    ...
    def check_roms_in_collection(self):
        """Check if ROMs from a text file exist in the selected collection"""
        try:
            # Get the selected collection
            selected_collection = self.collection_var.get()
            if selected_collection == "All Collections":
            ...
            file_path = filedialog.askopenfilename(
                title="Select ROM List File to Check",
            ...
            with open(file_path, 'r') as file:
                for line in file:
                    ...
                    # Check if the line contains a colon and extract the part after it
                    if ':' in line:
                        rom_name = line.split(':', 1)[1].strip().lower()
                    else:
                        rom_name = line.lower()
                        
                    if rom_name:
                        requested_roms.append(rom_name)

            if not requested_roms:
                messagebox.showinfo("Empty File", "The selected file doesn't contain any ROM names.")
                return

            # Get ROM extensions for the selected collection
            extensions = self.get_collection_extensions(selected_collection)
            
            # Get the ROM folder path for the selected collection
            rom_folder = self.get_collection_rom_path(selected_collection)
            
            if not rom_folder or not os.path.isdir(rom_folder):
                messagebox.showerror("Error", f"ROM folder not found for {selected_collection}.")
                return
                
            # Create a set of ROMs in the collection (lowercase for case-insensitive matching)
            collection_roms = set()
                ...
                if os.path.isfile(os.path.join(rom_folder, file)):
                    for ext in extensions:
                        if file.lower().endswith(ext.lower()):
                            ...
                            collection_roms.add(base_name)
                            break
            
            # Check each requested ROM against the collection
            found_roms = []
            missing_roms = []
            
            for rom in requested_roms:
                if rom.lower() in collection_roms:
                    found_roms.append(rom)
                else:
                    missing_roms.append(rom)
            
            # Calculate match percentage
            total_roms = len(requested_roms)
            match_percentage = (len(found_roms) / total_roms) * 100 if total_roms > 0 else 0
            
            # Show results in a nice dialog
            self.show_rom_check_results(found_roms, missing_roms, match_percentage, selected_collection, file_path)
            
        except Exception as e:
            import traceback
            traceback.print_exc()
            messagebox.showerror("Error", f"Error checking ROMs: {str(e)}")
            self.status_bar.configure(text="Error checking ROMs")
            ...
            # Show results in a nice dialog
            self.show_rom_check_results(found_roms, missing_roms, match_percentage, selected_collection)
    ...
    def show_rom_check_results(self, found_roms, missing_roms, match_percentage, collection_name, source_file_path):
        """Show the results of checking ROMs in a collection"""
        # Create results dialog
        results_window = tk.Toplevel()
        results_window.title(f"ROM Check Results - {collection_name}")
        results_window.configure(bg='#2c2c2c')
        ...
        window_width = max(window_width, 500)  # Minimum width
        window_height = max(window_height, 400)  # Minimum height
        ...
        # Main container
        main_frame = ctk.CTkFrame(results_window, fg_color='#2c2c2c')
        main_frame.pack(fill='both', expand=True, padx=10, pady=10)
        
        # Result header based on match percentage
        if match_percentage >= 90:
            header_color = '#4CAF50'  # Green for good match
            result_icon = "✓"
        elif match_percentage >= 50:
            header_color = '#FFC107'  # Yellow/amber for moderate match
            result_icon = "⚠"
        else:
            header_color = '#F44336'  # Red for poor match
            result_icon = "✗"
        
        # Results header
        header_frame = ctk.CTkFrame(main_frame, fg_color=header_color)
        header_frame.pack(fill='x', padx=0, pady=0)
        
        title_text = f"ROM Check Results: {result_icon} {match_percentage:.1f}% Match"
        title_label = ctk.CTkLabel(
            header_frame,
            text=title_text,
        ...
        # Summary stats
        stats_frame = ctk.CTkFrame(main_frame, fg_color='#333333')
        stats_frame.pack(fill='x', padx=5, pady=10)
        
        stats_text = (
            f"Collection: {collection_name}\n"
            f"ROMs Found: {len(found_roms)} of {len(found_roms) + len(missing_roms)}\n"
            f"ROMs Missing: {len(missing_roms)}"
        )
        ...
        # Create tabbed view for ROM lists
        tab_view = ctk.CTkTabview(main_frame, fg_color='#3c3c3c')
        tab_view.pack(fill='both', expand=True, padx=5, pady=10)
        
        # Add tabs
        tab_view.add("Found ROMs")
        tab_view.add("Missing ROMs")
        
        # Found ROMs tab
        found_text = ctk.CTkTextbox(
            tab_view.tab("Found ROMs"),
            fg_color='#3c3c3c'
        )
        found_text.pack(fill='both', expand=True, padx=5, pady=5)
        
        for rom in sorted(found_roms):
            found_text.insert('end', f"{rom}\n")
        found_text.configure(state='disabled')
        
        # Missing ROMs tab
        missing_text = ctk.CTkTextbox(
            tab_view.tab("Missing ROMs"),
            fg_color='#3c3c3c'
        )
        missing_text.pack(fill='both', expand=True, padx=5, pady=5)
        
        for rom in sorted(missing_roms):
            missing_text.insert('end', f"{rom}\n")
        missing_text.configure(state='disabled')
        
        # Set initial tab based on results
        if missing_roms:
            tab_view.set("Missing ROMs")
        else:
            tab_view.set("Found ROMs")
        ...
        on_close = self.main_app.show_popup(results_window)
        
        # Save results button
        def save_results():
            save_path = filedialog.asksaveasfilename(
                title="Save ROM Check Results",
            ...
            if save_path:
                try:
                    with open(save_path, 'w') as file:
                        file.write(f"ROM Check Results for {collection_name}\n")
                        file.write(f"Match Percentage: {match_percentage:.1f}%\n")
                        file.write(f"ROMs Found: {len(found_roms)} of {len(found_roms) + len(missing_roms)}\n")
                        file.write(f"ROMs Missing: {len(missing_roms)}\n\n")
                        
                        file.write("=== FOUND ROMS ===\n")
                        for rom in sorted(found_roms):
                            file.write(f"{rom}\n")
                    ...
                    messagebox.showinfo("Save Complete", f"Results saved to {save_path}")
                except Exception as e:
                    messagebox.showerror("Error", f"Error saving results: {str(e)}")
        
        # Function to create a temporary file with found ROMs and use it for removal
        def proceed_to_remove_unlisted_roms():
            try:
                # Create a temporary file with the found ROMs (to keep)
                temp_file = tempfile.NamedTemporaryFile(mode='w+', suffix='.txt', delete=False)
                temp_file_path = temp_file.name
                
                # Write the found ROMs to the temporary file
                for rom in found_roms:
                    temp_file.write(f"{rom}\n")
                temp_file.close()
                ...
                proceed = messagebox.askyesno(
                    "Remove Unlisted ROMs",
                    f"You are about to remove all ROMs from '{collection_name}' that are NOT in your checked list.\n\n"
                    f"This will keep {len(found_roms)} ROMs and remove all others.\n\n"
                    f"Do you want to proceed?",
                    icon=messagebox.WARNING
                )
                
                if proceed:
                    # Store the current collection selection to restore it later
                    current_collection = self.collection_var.get()
                    
                    # Create a temporary function to handle this specific case
                    def perform_remove_with_temp_file():
                        try:
                            # Read the ROMs to keep from the temporary file
                            with open(temp_file_path, 'r') as file:
                                roms_to_keep = [line.strip().lower() for line in file.readlines() if line.strip()]
                                
                            # Continue with the same logic as in continue_with_remove_roms_not_in_txt
                            ...
                            # Get ROM folder and extensions
                            rom_folder = self.get_collection_rom_path(collection_name)
                            extensions = self.get_collection_extensions(collection_name)
                            ...
                            # Create a dictionary of all ROMs in the folder
                            all_roms = {}
                            for filename in os.listdir(rom_folder):
                                if os.path.isfile(os.path.join(rom_folder, filename)):
                                    name_without_ext = os.path.splitext(filename)[0]
                                    file_ext = os.path.splitext(filename)[1].lower()
                                    if not extensions or file_ext in extensions:
                                        all_roms[name_without_ext.lower()] = os.path.join(rom_folder, filename)
                            
                            # Find ROMs to remove (all ROMs not in the keep list)
                            roms_to_remove = {}
                            for rom_name, rom_path in all_roms.items():
                                if rom_name not in roms_to_keep:
                                    roms_to_remove[rom_name] = rom_path
                            
                            # If no ROMs to remove, exit
                            if not roms_to_remove:
                                messagebox.showinfo("No ROMs to Remove", "All ROMs in the collection are in the keep list.")
                                return
                                
                            # Create destination folders
                            roms_removed_path = os.path.join(collection_path, 'roms_Removed')
                            
                            if not os.path.exists(roms_removed_path):
                                os.makedirs(roms_removed_path)
                            ...
                            # Move the ROMs
                            moved_count = 0
                            for rom_name, rom_path in roms_to_remove.items():
                                dest_path = os.path.join(roms_removed_path, os.path.basename(rom_path))
                                try:
                                    shutil.move(rom_path, dest_path)
                                    moved_count += 1
                                except Exception as e:
                                    print(f"Error moving ROM {rom_name}: {e}")
                            
                            # Close loading indicator
                            loading_popup.destroy()
                            
                            # Write logs
                            removal_log_path = os.path.join(roms_removed_path, '_removed_roms.txt')
                            with open(removal_log_path, 'w') as log_file:
                                for rom in sorted(roms_to_remove.keys()):
                                    log_file.write(f"{rom}\n")
                            
                            kept_log_path = os.path.join(roms_removed_path, '_kept_roms.txt')
                            with open(kept_log_path, 'w') as log_file:
                                for rom in sorted(roms_to_keep):
                                    log_file.write(f"{rom}\n")
                            
                            # Update status message
                            self.status_bar.configure(text=f"Moved {moved_count} ROMs to 'roms_Removed'")
                            
                            # Ask if they want to move artwork too
                            move_artwork_result = messagebox.askyesno(
                                "Move Artwork",
                                f"Do you also want to move the artwork for these {moved_count} removed ROMs?",
                                icon=messagebox.QUESTION
                            )
                            
                            if move_artwork_result:
                                # Create destination folder for artwork
                                artwork_removed_path = os.path.join(collection_path, 'medium_artworkRemoved')
                                if not os.path.exists(artwork_removed_path):
                                    os.makedirs(artwork_removed_path)
                                    
                                # Show loading indicator
                                loading_popup = self.show_loading_popup("Moving artwork...")
                                    
                                # Move the artwork
                                artwork_path = os.path.join(collection_path, 'medium_artwork')
                                moved_artwork_count = 0
                                
                                if os.path.exists(artwork_path):
                                    # Move artwork for removed ROMs
                                    for subfolder in os.listdir(artwork_path):
                                        subfolder_path = os.path.join(artwork_path, subfolder)
                                        if os.path.isdir(subfolder_path):
                                            # Create destination subfolder
                                            dest_subfolder = os.path.join(artwork_removed_path, subfolder)
                                            
                                            for file in os.listdir(subfolder_path):
                                                file_base_name = os.path.splitext(file)[0].lower()
                                                if file_base_name in roms_to_remove:
                                                    ...
                                                    try:
                                                        ...
                                                        moved_artwork_count += 1
                                                    except Exception as e:
                                                        print(f"Error moving artwork for {file_base_name}: {e}")
                                
                                # Close loading indicator
                                loading_popup.destroy()
                                
                                # Update status and show success message
                                self.status_bar.configure(text=f"Moved {moved_count} ROMs and {moved_artwork_count} artwork files")
                                messagebox.showinfo("Operation Complete", 
                                                f"Successfully moved {moved_count} ROMs to 'roms_Removed' and {moved_artwork_count} artwork files to 'medium_artworkRemoved'.")
                            else:
                                messagebox.showinfo("Operation Complete", 
                                                f"Successfully moved {moved_count} ROMs to 'roms_Removed'.")
                    ...
                    perform_remove_with_temp_file()
                    
            except Exception as e:
                import traceback
                traceback.print_exc()
                messagebox.showerror("Error", f"Error preparing for ROM removal: {str(e)}")
        
        # Remove ROMs button - only if there are found ROMs
        if found_roms:
            remove_button = ctk.CTkButton(
                button_frame,
                text="Remove ROMs Not In List",
                command=proceed_to_remove_unlisted_roms,
                font=('Helvetica', 12),
                fg_color='#FF5722',  # Deep orange
                hover_color='#E64A19'
            )
        ...
        close_button = ctk.CTkButton(
            button_frame,
            text="Close",
            command=lambda: on_close() if on_close else results_window.destroy(),
            font=('Helvetica', 12),
            fg_color='#757575',
            hover_color='#616161'
        )
    ...
    def validate_rom_list_for_collection(self, rom_list, selected_collection):
        """
        Validate that the ROMs in the list belong to the selected collection.
        Returns a tuple of (matching_roms, non_matching_roms) and a match percentage.
        """
        try:
            # Get ROM extensions for the selected collection
            extensions = self.get_collection_extensions(selected_collection)
            ...
            if not rom_folder or not os.path.isdir(rom_folder):
                return [], rom_list, 0  # No valid ROM folder
            ...
            # Check each ROM in the list against the collection
            matching_roms = []
            non_matching_roms = []
            
            for rom in rom_list:
                rom_lower = rom.lower()
                if rom_lower in collection_roms:
                    matching_roms.append(rom)
                else:
                    non_matching_roms.append(rom)
            
            # Calculate match percentage
            total_roms = len(rom_list)
            if total_roms > 0:
                match_percentage = (len(matching_roms) / total_roms) * 100
            else:
                match_percentage = 0
                
            return matching_roms, non_matching_roms, match_percentage
            
        except Exception as e:
            print(f"Error validating ROM list: {str(e)}")
            return [], rom_list, 0  # Return as if no matches on error

    def get_collection_extensions(self, collection_name):
        """Get the file extensions for a collection from its settings.conf"""
        try:
            root_dir = PathManager.get_base_path()
            collections_dir = os.path.join(root_dir, 'collections')
            collection_path = os.path.join(collections_dir, collection_name)
            ...
            extensions = ['.zip']  # Default extension if not specified
            
            if os.path.isfile(settings_path):
                with open(settings_path, 'r') as settings_file:
                    for line in settings_file:
                        line = line.strip()
                        if line.startswith("list.extensions"):
                            ext_line = line.split("=", 1)[1].strip()
                            extensions = [
            ...
            print(f"Error getting collection extensions: {str(e)}")
            return ['.zip']  # Return default on error

    def get_collection_rom_path(self, collection_name):
        """Get the ROM path for a collection from its settings.conf"""
        try:
            root_dir = PathManager.get_base_path()
            collections_dir = os.path.join(root_dir, 'collections')
            collection_path = os.path.join(collections_dir, collection_name)
            ...
            print(f"Error getting collection ROM path: {str(e)}")
            return None  # Return None on error

    def show_validation_warning(self, matching, non_matching, match_percentage, selected_collection, operation_type="move"):
        """Show a warning dialog if the ROM list doesn't match the selected collection"""
        if not non_matching:
            # All ROMs match the collection, no warning needed
            return True
            
        # Create warning dialog
        warning_window = tk.Toplevel()
        warning_window.title("ROM List Validation Warning")
        warning_window.configure(bg='#2c2c2c')
        
        # Make it a modal dialog
        warning_window.transient(self.parent_tab.winfo_toplevel())
        ...
        header_frame = ctk.CTkFrame(warning_window, fg_color='#f44336')
        header_frame.pack(fill='x', padx=0, pady=0)
        
        title_text = "Warning: ROM List Mismatch"
        title_label = ctk.CTkLabel(
            header_frame,
            text=title_text,
            font=('Helvetica', 18, 'bold'),
        ...
        # Warning message
        action_text = "move" if operation_type == "move" else "keep"
        message_frame = ctk.CTkFrame(warning_window, fg_color='#2c2c2c')
        message_frame.pack(fill='both', expand=True, padx=20, pady=10)
        
        message = (
            f"You've selected the '{selected_collection}' collection, but {len(non_matching)} out of {len(matching) + len(non_matching)} "
            f"ROMs in your text file ({match_percentage:.1f}% match) don't exist in this collection.\n\n"
            f"This could mean you're trying to {action_text} ROMs for a different collection."
        )
        ...
        # Create tabbed view for ROM lists
        tab_view = ctk.CTkTabview(message_frame, fg_color='#3c3c3c')
        tab_view.pack(fill='both', expand=True, padx=10, pady=10)
        
        # Add tabs
        tab_view.add("Non-Matching ROMs")
        tab_view.add("Matching ROMs")
        
        # Non-matching ROMs tab
        non_matching_text = ctk.CTkTextbox(
            tab_view.tab("Non-Matching ROMs"),
            fg_color='#3c3c3c'
        )
        non_matching_text.pack(fill='both', expand=True, padx=5, pady=5)
        
        for rom in sorted(non_matching):
            non_matching_text.insert('end', f"{rom}\n")
        non_matching_text.configure(state='disabled')
        
        # Matching ROMs tab
        matching_text = ctk.CTkTextbox(
            tab_view.tab("Matching ROMs"),
            fg_color='#3c3c3c'
        )
        matching_text.pack(fill='both', expand=True, padx=5, pady=5)
        
        for rom in sorted(matching):
            matching_text.insert('end', f"{rom}\n")
        matching_text.configure(state='disabled')
        
        # Buttons frame
        button_frame = ctk.CTkFrame(warning_window, fg_color='#2c2c2c')
        ...
        proceed_button = ctk.CTkButton(
            ...
            command=on_proceed,
            font=('Helvetica', 12),
            fg_color='#ff9800',  # Orange for caution
            hover_color='#e68a00'
        )
        proceed_button.pack(side='right', padx=5)
        ...
        cancel_button = ctk.CTkButton(
            ...
            command=on_cancel,
            font=('Helvetica', 12),
            fg_color='#4CAF50',  # Green for safe option
            hover_color='#45a049'
        )
    ...
    def show_rom_operations_selection(self):
        ...
        try:
            ...
            selection_window = tk.Toplevel(self.parent_tab)
            selection_window.title("ROM Operations")
            ...
            # Create a main frame with scrollable content
            main_frame = ctk.CTkFrame(selection_window, fg_color='#2c2c2c')
            main_frame.pack(fill='both', expand=True, padx=10, pady=10)
            
            # Add title label inside main frame
            title_label = ctk.CTkLabel(
                main_frame,
                text=f"ROM Operations for {selected_collection}",
            ...
            description = ctk.CTkLabel(
                main_frame,
                text="Select which operation you want to perform:",
                wraplength=window_width - 40,
                text_color='white',
                font=('Helvetica', 14)
            )
            ...
            # Create a scrollable canvas for the buttons
            canvas_frame = ctk.CTkFrame(main_frame, fg_color='#2c2c2c')
            ...
            button_frame = ctk.CTkFrame(canvas, fg_color='#2c2c2c')
            button_frame.pack(fill='x', expand=True, padx=10, pady=5)
            
            # Add the button frame to the canvas
            canvas.create_window((0, 0), window=button_frame, anchor='nw', width=window_width-50)
            ...
            cancel_button.pack(fill='x', padx=10)
            
            # Update the canvas scroll region when the button frame changes size
    ...
    def refresh_rom_list(self):
        ...
        try:
            # Store current collection selection
            current_collection = self.collection_var.get()
            ...
            if current_search or current_collection != "All Collections":
                self.search_var.set(current_search)
                self.collection_var.set(current_collection)
                self.sort_var.set(current_sort)
                self.filter_roms()
                
            self.status_bar.configure(text="ROM list refreshed")
            
        except Exception as e:
            messagebox.showerror("Error", f"Error refreshing ROM list: {str(e)}")
            self.status_bar.configure(text="Error refreshing ROM list")
    
    def remove_roms_not_in_txt(self):
        """Move all ROMs that are NOT listed in a text file, along with their artwork"""
        try:
            # Get the selected collection
            selected_collection = self.collection_var.get()
            if selected_collection == "All Collections":
            ...
            show_popup_flag = self.config_manager.get_setting('Settings', 'show_remove_roms_not_in_txt_instructions', 'True')
            if show_popup_flag == 'True':
                ...
                title_label = ctk.CTkLabel(
                    popup,
                    text="Instructions for Removing ROMs Not In Text File",
                    wraplength=500,
                    text_color='white',
                    font=('Helvetica', 18, 'bold')
                )
                ...
                instructions_text.insert('1.0', 
                    "1. You have selected the Collection: " + selected_collection + "\n\n"
                    "2. Next, you will be prompted to select a text file containing the ROM names you want to KEEP.\n\n"
                    "3. In the text file, each ROM to keep should be on a new line. Do not add file extensions.\n\n"
                    "4. All ROMs in the Collection that are NOT listed in the text file will be moved to the 'roms_Removed' folder.\n\n"
                    "5. All artwork for the removed ROMs will be moved to the 'medium_artworkRemoved' folder.\n\n"
                    "This is useful for cleaning up collections by specifying only the ROMs you want to keep rather than the ones to remove."
                )
                ...
                def on_ok():
                    if do_not_show_var.get():
                        self.config_manager.config.set('Settings', 'show_remove_roms_not_in_txt_instructions', 'False')
                        self.config_manager.save_config()
                    if on_close:
                        on_close()
                    self.continue_with_remove_roms_not_in_txt(selected_collection)
            ...
            import traceback
            traceback.print_exc()
            messagebox.showerror("Error", f"Error in remove_roms_not_in_txt: {str(e)}")
            self.status_bar.configure(text="Error preparing to remove ROMs")

    def continue_with_remove_roms_not_in_txt(self, selected_collection):
        """Continue with the ROM removal process after instructions"""
        try:
            # Prompt the user to select the text file containing the ROMs to keep
            file_path = filedialog.askopenfilename(
                title="Select ROM List File (ROMs to KEEP)",
                filetypes=[("Text Files", "*.txt")]
            )
            if not file_path:
                messagebox.showinfo("Cancelled", "Operation cancelled by the user.")
            ...
            # Read the list of ROM names to keep from the text file
            with open(file_path, 'r') as file:
                roms_to_keep = [line.strip().lower() for line in file.readlines() if line.strip()]
                
            # Validate that the ROMs are for the selected collection
            matching_roms, non_matching_roms, match_percentage = self.validate_rom_list_for_collection(
                roms_to_keep, selected_collection
            )
            
            # If there are non-matching ROMs, show a warning
            if non_matching_roms and match_percentage < 90:  # Warning threshold - 90%
                # Show warning with "keep" operation type since these are ROMs to keep
                proceed = self.show_validation_warning(
                    matching_roms, non_matching_roms, match_percentage, 
                    selected_collection, operation_type="keep"
                )
                if not proceed:
            ...
            def create_scrollable_confirmation():
                ...
                confirm_window.title(f"Confirm ROM Removal - {selected_collection}")
                ...
                label = ctk.CTkLabel(
                    confirm_window,
                    text=f"Are you sure you want to move these {len(roms_to_remove)} ROMs not in the keep list?\n\nThey will be moved to 'roms_Removed' and their artwork to 'medium_artworkRemoved'.",
                    wraplength=480,
                    text_color='white',
                    font=('Helvetica', 14)
                )
                ...
                stats_label = ctk.CTkLabel(
                    confirm_window,
                    text=f"Collection: {selected_collection}\nTotal ROMs: {len(all_roms)}\nROMs to keep: {len(roms_to_keep)}\nROMs to move: {len(roms_to_remove)}",
                    wraplength=480,
                    text_color='white',
                    font=('Helvetica', 12)
                )
            ...
            def proceed_with_remove():
                ...
                if move_artwork_result:
                    move_artwork_for_removed_roms(roms_to_remove, moved_count)
                else:
                    messagebox.showinfo("Operation Complete", 
                                    f"Successfully moved {moved_count} ROMs to 'roms_Removed'.")
                    # Add this line:
            ...
            def move_artwork_for_removed_roms(removed_roms, rom_count):
                # Create destination folder for artwork
                artwork_removed_path = os.path.join(collection_path, 'medium_artwork_Removed')
                if not os.path.exists(artwork_removed_path):
                    os.makedirs(artwork_removed_path)
                ...
                if os.path.exists(artwork_path):
                    ...
                    for subfolder in os.listdir(artwork_path):
                        ...
                        if os.path.isdir(subfolder_path):
                            ...
                            for file in os.listdir(subfolder_path):
                                file_base_name = os.path.splitext(file)[0].lower()
                                if file_base_name in removed_roms:
                                    # Create destination subfolder if it doesn't exist
                                    if not os.path.exists(dest_subfolder):
                                        os.makedirs(dest_subfolder)
                ...
                self.refresh_rom_list()
                # Update status and show success message
                self.status_bar.configure(text=f"Moved {rom_count} ROMs and {moved_artwork_count} artwork files")
                messagebox.showinfo("Operation Complete", 
                                f"Successfully moved {rom_count} ROMs to 'roms_Removed' and {moved_artwork_count} artwork files to 'medium_artworkRemoved'.")
            ...
            import traceback
            traceback.print_exc()
            messagebox.showerror("Error", f"Error removing ROMs not in list: {str(e)}")
            self.status_bar.configure(text="Error removing ROMs")
    ...
    def select_games_to_remove(self):
        ...
        try:
            ...
            def confirm_removal():
                ...
                # Add this line to refresh the ROM list
                self.refresh_rom_list()
    ...
    def move_roms(self, rom_list=None):
        ...
        try:
            ...
            if rom_list is None:
                ...
                # Read the list of ROM names from the text file
                with open(file_path, 'r') as file:
                    rom_list = [line.strip() for line in file.readlines() if line.strip()]
                    
                # Validate that the ROMs are for the selected collection
                matching_roms, non_matching_roms, match_percentage = self.validate_rom_list_for_collection(
                    rom_list, selected_collection
                )
                ...
                if non_matching_roms and match_percentage < 90:  # Warning threshold - 90%
                    proceed = self.show_validation_warning(
                        matching_roms, non_matching_roms, match_percentage, 
                        selected_collection, operation_type="move"
                    )
                    if not proceed:
                        return  # User chose to cancel the operation
            ...
            def proceed_with_move():
                ...
                # Show artwork move confirmation
                move_artwork_result = messagebox.askyesno(
                    "Move Artwork",
                    f"Do you also want to move the artwork for the {moved_count} moved ROMs?",
                    icon=messagebox.QUESTION
                )
                ...
                if move_artwork_result:
                    ...
                    # Add refresh here since artwork won't be moved
                    self.refresh_rom_list()
